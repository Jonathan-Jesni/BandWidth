...
produce a structured JSON review, posts it (mentioning only the silent Architect),
then sends a second direct message to only the Tester/Engineer with the verdict.
"""
...
# Upgrade: Force the LLM to output strict JSON
_SYSTEM_PROMPT = """\
...
class ReviewerAdapter(SimpleAdapter[Any]):
    ...
    def _tester_mention(self, tools: AgentToolsProtocol) -> list[str]:
        """Mention only the Tester/Engineer for the verdict notification."""
        self_id = self._self_id()
        try:
            architect_id = config.architect().agent_id
        except Exception:
    ...
    async def on_message(
        ...
        *,
        ...
        try:
            # Upgrade: Added response_format={"type": "json_object"}
            ...
            # Upgrade: Parse the JSON to guarantee exact match
        ...
        # so we use a second send_message with only the Tester/Engineer as recipient.
        ...
        if tester_mentions:
            ...
            log.info("[Reviewer] Verdict notification sent downstream")